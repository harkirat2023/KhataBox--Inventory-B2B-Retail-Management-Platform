...
const statusConfig: Record<string, { label: string; class: string }> = {
  pending: { label: "Pending", class: "bg-amber-600 text-white dark:bg-amber-500 dark:text-white" },
  approved: { label: "Approved", class: "bg-blue-600 text-white dark:bg-blue-500 dark:text-white" },
  rejected: { label: "Rejected", class: "bg-red-600 text-white dark:bg-red-500 dark:text-white" },
  completed: { label: "Completed", class: "bg-green-600 text-white dark:bg-green-500 dark:text-white" },
}
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <Select value={statusFilter} onValueChange={(val) => val && setStatusFilter(val)}>
            <SelectTrigger className="w-[150px] rounded-xl border-border h-11">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 bg-card rounded-2xl border border-border shadow-sm">
            <ArrowLeftRight className="size-12 text-muted-foreground/30 mb-4" />
            <h3 className="text-lg font-semibold text-foreground">No transfers found</h3>
            <p className="text-sm text-muted-foreground mt-1 mb-6">Create your first stock transfer to get started.</p>
            <Button className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl h-11 px-5 transition-all duration-200" onClick={() => setDialogOpen(true)}>
                ...
                <TableCell>
                  {t.status === "pending" && (
                    <div className="flex gap-1">
                      <Button size="sm" className="bg-card border border-border text-foreground/80 hover:bg-muted rounded-lg h-8 px-3 text-xs transition-all duration-200" onClick={() => handleApprove(t.id, "approved")}>
                        Approve
                      </Button>
                      <Button size="sm" className="bg-card border border-border text-destructive hover:bg-destructive/10 rounded-lg h-8 px-3 text-xs transition-all duration-200" onClick={() => handleApprove(t.id, "rejected")}>
                        Reject
                      </Button>
                    </div>
                  )}
                  {t.status === "approved" && (
                    <Button size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg h-8 px-3 text-xs transition-all duration-200" onClick={() => handleApprove(t.id, "completed")}>
                      Complete
                    </Button>
                  )}
                </TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div>
              <label className="text-sm font-medium text-foreground/80 mb-1.5 block">Product *</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-foreground/80 mb-1.5 block">To Store *</label>
            ...
            <div>
              <label className="text-sm font-medium text-foreground/80 mb-1.5 block">Quantity *</label>
              <Input type="number" min={1} value={form.quantity || ""} onChange={(e) => setForm({ ...form, quantity: parseInt(e.target.value) || 0 })} className="rounded-xl border-border h-11" inputMode="numeric" />
            </div>
            <div>
          ...
          <DialogFooter>
            ...
            <Button className="gap-1.5 bg-green-600 hover:bg-green-700 text-white rounded-xl h-11 px-5" onClick={handleCreate} disabled={creating}>{creating ? "Creating..." : "Create Transfer"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
