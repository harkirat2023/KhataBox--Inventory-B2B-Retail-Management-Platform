    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center">
        <div className="text-center">
          <div className="size-16 mx-auto mb-4 rounded-2xl bg-blue-50 flex items-center justify-center animate-pulse">
      ...
      <div className="px-4 mt-4">
        <div className="bg-slate-100 rounded-2xl p-1 flex">
          <button onClick={() => setActiveTab("active")}
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${activeTab === "active" ? "bg-white shadow-sm text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
            Active ({activeOrders.length})
          </button>
          <button onClick={() => setActiveTab("past")}
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${activeTab === "past" ? "bg-white shadow-sm text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
      ...
      <div className="px-4 mt-4 space-y-3 pb-4">
        {orders.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center shadow-sm">
            <div className="size-16 mx-auto mb-3 rounded-2xl bg-slate-50 flex items-center justify-center">
              <Package className="size-8 text-slate-300" />
            </div>
            <h2 className="font-semibold text-slate-900">No orders yet</h2>
            <p className="text-sm text-slate-500 mt-1">Your orders will appear here</p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center shadow-sm">
            <div className="size-14 mx-auto mb-3 rounded-2xl bg-slate-50 flex items-center justify-center">
          ...
          displayOrders.map((order) => {
            ...
            return (
              <Link key={order.id} href={`/my-orders/${order.id}`}>
                <div className="bg-white rounded-2xl border border-slate-200 p-4 hover:shadow-lg hover:border-slate-300 transition-all duration-200 group shadow-sm">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="font-mono font-semibold text-sm text-slate-900">{order.order_number}</p>
                        <ChevronRight className="size-4 text-slate-300 group-hover:text-slate-400 transition-colors" />
                      </div>
                      ...
                      {order.store_name && (
                        <p className="text-[10px] text-slate-400 mt-1">{order.store_name}</p>
                      )}
                    </div>
                  </div>
                </div>
