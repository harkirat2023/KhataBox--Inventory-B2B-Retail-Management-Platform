...
function CartContent() {
  ...
  useEffect(() => {
    async function fetchCart() {
      ...
      try {
        const carts = await clientApi.get<ServerCart[]>("/api/v1/cart/")
  ...
  const handleRemoveItem = async (productId: number) => {
    ...
    setSubmitting(true)
    try {
      await clientApi.delete(`/api/v1/cart/${serverCart.id}/items/${productId}`)
  ...
  const handleUpdateQuantity = async (productId: number, newQty: number) => {
    ...
    try {
      ...
      if (item) {
        await clientApi.put(`/api/v1/cart/${serverCart.id}/items/${item.id}?quantity=${newQty}`, {})
  ...
  const handleCheckout = async () => {
    setCheckingOut(true)
    try {
      const result = await clientApi.post<any>("/api/v1/cart/checkout", {
