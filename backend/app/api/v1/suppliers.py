...
async def list_suppliers(
    ...
    base_query = select(Supplier).where(Supplier.owner_id == current_user.id)
    total = None
    if page is not None and page_size is not None:
        count_result = await db.execute(select(func.count()).select_from(Supplier).where(Supplier.owner_id == current_user.id))
