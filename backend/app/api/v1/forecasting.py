    ...
    try:
        ...
        if ml_predict.is_model_ready():
            try:
                ...
                logger.warning("ML prediction failed, using fallback: %s", ml_err)
        if forecast is None:
        ...
        logger.critical("Forecast error for product %s: %s", product_id, exc, exc_info=True)
...
async def get_sales_history(
    ...
    try:
        ...
        logger.critical("Sales history error for product %s: %s", product_id, exc, exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Sales history failed: {exc}")
