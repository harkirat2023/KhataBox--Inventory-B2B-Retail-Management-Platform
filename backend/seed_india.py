...
# ---------------------------------------------------------------------------
# Seed Product Catalog  (per store type for new shopkeeper onboarding)
# ---------------------------------------------------------------------------
SEED_PRODUCT_CATALOG = {
    ...
    "restaurant": [
        ...
        ("Tissue Paper Pack 10", "TISU", "Home & Kitchen", 180, 100),
        ("Aluminium Foil Roll", "ALUM", "Home & Kitchen", 120, 60),
        ("Cling Wrap Roll", "CLNG", "Home & Kitchen", 80, 40),
    ],
}
...
async def seed():
    ...
    async with async_session() as session:
        ...
        if existing_admin:
            ...
            # Disable FK checks temporarily to allow unordered DELETE
        ...
        for d in date_range:
            ...
            for _ in range(num_orders_today):
                ...
                status_weights = [10, 5, 3, 10, 60, 10] if d < end_date else [40, 20, 15, 20, 5, 10]
        ...
        # ---------------------------------------------------------------
        # 12. SEED PRODUCTS  (for shopkeeper onboarding)
        # ---------------------------------------------------------------
        print("Seeding SeedProducts...")
        seed_products_created = 0
        for store_type, items in SEED_PRODUCT_CATALOG.items():
            for name, sku_prefix, category, selling_price, cost_price in items:
                result = await session.execute(
                    select(SeedProduct).where(
                        SeedProduct.store_type == store_type,
                        SeedProduct.name == name,
                    )
                )
                ...
                session.add(sp)
                seed_products_created += 1
        print(f"  {seed_products_created} seed products across {len(SEED_PRODUCT_CATALOG)} store types")
