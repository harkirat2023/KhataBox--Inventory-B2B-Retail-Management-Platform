...
describe("clientApi.get", () => {
  it("calls fetch with GET and returns JSON", async () => {
    ...
    const getCall = mockFetch.mock.calls.find((c) => c[0] === "http://localhost:8000/test")
    expect(getCall).toBeDefined()
  })

  it("throws on non-ok response", async () => {
    mockFetch.mockResolvedValueOnce({ ok: false, status: 404 })
    await expect(clientApi.get("/test")).rejects.toThrow("API error: 404")
  })
})
...
describe("clientApi.post", () => {
  it("calls fetch with POST and body", async () => {
    ...
    expect(mockFetch.mock.calls[0][0]).toBe("http://localhost:8000/create")
...
describe("clientApi.put", () => {
  it("calls fetch with PUT", async () => {
    ...
    expect(mockFetch.mock.calls[0][0]).toBe("http://localhost:8000/update/1")
...
describe("clientApi.delete", () => {
  it("calls fetch with DELETE", async () => {
    ...
    expect(mockFetch.mock.calls[0][0]).toBe("http://localhost:8000/delete/1")
...
describe("clientApi.patch", () => {
  it("calls fetch with PATCH", async () => {
    ...
    expect(mockFetch.mock.calls[0][0]).toBe("http://localhost:8000/patch/1")
